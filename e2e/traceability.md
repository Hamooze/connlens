...
|---------|------------------|--------------|----------|--------|
| S1-F1 | Fixture scan renders 5 groups / 8+ rows within 5 seconds | 1.1.1-1.1.5, 1.2.2 | `tests/fixtures/home`, pending Tauri visual run | PARTIAL |
...
| S1-F3 | MCP servers from Claude Desktop, Claude Code, Cursor with distinct scopes | 1.1.4 | fixture files, pending scanner E2E | PARTIAL |
| S1-F4 | Search narrows by provider/label/host and clear restores | 1.2.2 | `npm test` connection view helpers | AUTOMATED |
| S1-F5 | Expand row, copy, open dashboard, reveal source, hide | 1.2.3 | React UI wired to IPC/dev action wrappers; pending Tauri visual run | PARTIAL |
| S1-F6 | Hide row; show hidden restores; active remove blocked | 1.2.3, 1.1.1 | `cargo test registry::tests::active_auto_detected_rows_are_not_removable` | PARTIAL |
...
| S1-F11 | Restart persists history/hidden/collapse and second launch focuses | 1.1.1, 1.2.1 | registry tests; single-instance configured, pending shell E2E | PARTIAL |
